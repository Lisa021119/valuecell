...
export default function LoginModal({ children }: LoginModalProps) {
  ...
  const handleLogin = async (provider: PendingAction) => {
    ...
    const authBase = import.meta.env.VITE_AUTH_LOGIN_BASE_URL?.trim() ?? "";
    if (!authBase) {
      toast.error(t("auth.login.errors.cloudDisabled.title"), {
        description: t("auth.login.errors.cloudDisabled.description"),
      });
      return;
    }

    try {
      await openUrl(`${authBase.replace(/\/$/, "")}/login?provider=${provider}`);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <div className="mt-10 flex flex-col items-center gap-3 text-center">
          ...
          />
          <p className="font-medium text-3xl text-foreground">
            {t("product.name")}
          </p>
