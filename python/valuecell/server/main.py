...
def main() -> None:
    ...
    # Skip stdin control when:
    # - ENV=local_dev: PyCharm/IDE debug wrappers can break iterating sys.stdin
    # - stdin is not a TTY: background/piped processes get EOF immediately and
    #   would otherwise shut down the server on startup (see control_loop).
    if os.getenv("ENV") == "local_dev" or not sys.stdin.isatty():
        if os.getenv("ENV") == "local_dev":
            ...
            logger.info(
                "Non-interactive stdin: skipping stdin control thread (background mode)"
            )
        control_thread = None
    else:
