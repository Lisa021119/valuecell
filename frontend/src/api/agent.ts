...
import { INSIGHTDESK_HUB_AGENT } from "@/constants/agent";
...
export const useGetAgentInfo = (params: { agentName: string }) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Return hardcoded data for hub agent (local demo / SSR).
      if (params.agentName === "InsightDeskAgent") {
        return Promise.resolve({ data: INSIGHTDESK_HUB_AGENT });
      }
