...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        # Startup
        logger.info(
            f"{settings.APP_NAME} starting up on {settings.API_HOST}:{settings.API_PORT}..."
        )
        ...
        yield
        # Shutdown
        logger.info("{name} shutting down...", name=settings.APP_NAME)

    app = FastAPI(
        title=f"{settings.APP_NAME} API",
        description="Multi-agent market intelligence API — ENMGT 5400 InsightDesk prototype",
...
def _add_routes(app: FastAPI, settings) -> None:
    ...
    async def home_page():
        return SuccessResponse.create(
            ...
            msg=f"Welcome to {settings.APP_NAME} API",
        )

    @app.get(f"{API_PREFIX}/healthz", response_model=SuccessResponse)
    async def health_check():
        return SuccessResponse.create(msg=f"Welcome to {settings.APP_NAME}!")
