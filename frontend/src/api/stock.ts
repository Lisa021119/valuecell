  ...
  useQuery({
    ...
    queryFn: () =>
      apiClient.get<ApiResponse<StockHistory[]>>(
        `${REMOTE_SAAS_API_BASE}/watchlist/asset/${params.ticker}/price/historical?interval=${params.interval}&start_date=${params.start_date}&end_date=${params.end_date}`,
        {
          requiresAuth: true,
        },
      ),
    select: (data) => data.data,
    enabled: !!params.ticker && HAS_REMOTE_SAAS_API,
  });
