...
**InsightDesk** uses a **three-tier configuration system** for development through production-style deployments. This guide covers agents, providers, and models.

## Note for the ENMGT 5400 build

- **Never commit** real `.env` files, API keys, or exchange secrets to Git.
- Runtime paths on disk may still use the legacy **`ValueCell`** folder name under Application Support (see below); that is normal and keeps existing installs working.

### Where the runtime env file lives

- `start.sh` / `start.ps1` load environment from the **OS application directory** (same area as the SQLite DB). Typical paths:
  - **macOS:** `~/Library/Application Support/ValueCell/.env`
  - **Linux:** `~/.config/valuecell/.env`
  - **Windows:** `%APPDATA%\ValueCell\.env`
- The repository root **`.env.example`** is the template: copy its variables into that runtime `.env` (or symlink if you prefer keeping one file under version control **without** secrets — still exclude secrets from Git).

## Configuration Priority

The app resolves configuration from multiple sources in this order (highest to lowest priority):
...
InsightDesk supports multiple LLM providers. Choose at least one:
...
1. Open the template at the **repository root**: `.env.example`.
2. Create your **runtime** `.env` in the OS application directory (see [Where the runtime env file lives](#where-the-runtime-env-file-lives)), **or** copy for local editing:

```bash
# From repository root — template only; fill in your keys locally
cp .env.example .env
```

3. Set at least **one** LLM provider key, for example:
...
4. Move or merge the filled values into the path your launcher reads (`start.sh` uses the ValueCell app-support path on first run).
