...
/** Optional SaaS API base (auth refresh, analytics). Empty = classroom / local-only build. */
const _remote = import.meta.env.VITE_REMOTE_SAAS_API_BASE?.trim();
export const REMOTE_SAAS_API_BASE =
  _remote !== undefined && _remote.length > 0 ? _remote : "";

/** True when optional cloud SaaS URL is configured (OAuth leaderboard / hosted extras). */
export const HAS_REMOTE_SAAS_API = REMOTE_SAAS_API_BASE.length > 0;
